...
export function ExecutionLogPanel({ taskId, onCompleted }: { taskId: string; onCompleted: () => void }) {
  ...
  const load = async () => setLogs(await getExecutionLogs(taskId));
  ...
  return <div className="execution-panel">
    ...
    {logs.map((l)=><div className="execution-row" key={l.id}><span>{new Date(l.startedAt).toLocaleString("ko-KR")} ~ {l.endedAt ? new Date(l.endedAt).toLocaleString("ko-KR") : "진행 중"}</span><b>{l.actualMinutes}분</b>{l.blockedReason && <em>막힘: {l.blockedReason}</em>}</div>)}
  </div>;
}
